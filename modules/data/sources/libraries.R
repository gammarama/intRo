...
if (!("package:DT" %in% search())) interpolate(~(library(DT)), file = "code_All.R", mydir = userdir, append = TRUE, nodupes = TRUE)
if (!("package:htmlwidgets" %in% search())) interpolate(~(library(htmlwidgets)), file = "code_All.R", mydir = userdir, append = TRUE, nodupes = TRUE)
