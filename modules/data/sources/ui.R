sources_ui <- tabPanel("Sources",
                                                  column(4,
                                                         wellPanel(
                                                             ...
                                                             tags$button("", id = "savesubset", type = "button", class = "btn action-button", onclick="var vals = []; var subsets = $('input[type = \"text\"][placeholder]'); for(i = 0; i < subsets.length; i++) {vals.push(subsets[i].value);}; Shiny.onInputChange(\"subs\", vals); $('#side-nav :contains(\"Sources\")').highlight(); $('a:has(> .fa-print, .fa-code, .fa-download)').hightlight();", list(icon("save"), "Save Subset")),
                                                             br(), br(),
