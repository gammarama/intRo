...
intro.quantdata <- function(intro.plot) {
    ...
    ", env = environment(), file = "code_graphical_reactive.R", append = TRUE)
    
    return(intro.quant)
}

input_xdomaindata <- function(xmin, xmax) {
    cat_and_eval(paste0("input_xdomain <- c(", xmin, ", ", xmax, ")"), env = environment(), file = "code_graphical_reactive.R", append = TRUE)
    
    return(input_xdomain)
}

input_ydomaindata <- function(ymin, ymax) {
    cat_and_eval(paste0("input_ydomain <- c(", ymin, ", ", ymax, ")"), env = environment(), file = "code_graphical_reactive.R", append = TRUE)
    
    return(input_ydomain)
}

input_binwidthdata <- function(binwidth) {
    if (is.na(binwidth)) cat_and_eval("input_binwidth <- NULL", env = environment(), file = "code_graphical_reactive.R", append = TRUE)
    else cat_and_eval(paste0("input_binwidth <- ", binwidth), env = environment(), file = "code_graphical_reactive.R", append = TRUE)
