my.lm <- function (intro.data, x, y) {
    interpolate(
        ~assign("intro.regression", lm(df$x ~ df$y, na.action = na.exclude)),
        df = quote(intro.data),
        x = x,
        y = y,
        mydir = userdir,
...
my_regdata <- function(intro.data, x, y) {
    interpolate(
        ~assign("reg.data", cbind(df, xreg = df$x, yreg = df$y, id = 1:nrow(df))),
        df = quote(intro.data),
        x = x,
        y = y,
        mydir = userdir,
...
my_regresid1 <- function(intro.data, intro.regression, x) {
    interpolate(
        ~assign("reg.resid1", data.frame(id = 1:length(resid(myreg)), residuals = resid(myreg), x = df[as.numeric(names(resid(myreg))), x])),
        df = quote(intro.data),
...
my_regresid2 <- function(intro.data, intro.regression) {
    interpolate(
        ~assign("reg.resid2", data.frame(id = as.numeric(names(na.omit(resid(myreg)))), 
...
savefit <- function (intro.data, intro.regression) { 
    ...
    interpolate(
        ~assign("intro.data", cbind(df, fitted = predict(myreg), resid = resid(myreg))),
        df = quote(intro.data),
        myreg = quote(intro.regression),
        mydir = userdir,
        `_env` = environment(),
